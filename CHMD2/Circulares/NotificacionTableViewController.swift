//
//  NotificacionTableViewController.swift
//  CHMD2
//
//  Created by Rafael David Castro Luna on 8/3/20.
       ...
       func contextualDelAction(forRowAtIndexPath indexPath: IndexPath) -> UIContextualAction {
        ...
        func borraCirculares(idCircular:Int,idUsuario:Int){
            ...
            if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                ...
                let query = "UPDATE appCircularCHMD SET eliminada=1,leida=0,favorita=0 WHERE idCircular=? AND idUsuario=? AND tipo=2"
        ...
        func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            ...
            UserDefaults.standard.set(indexPath.row,forKey:"posicion")
                ...
                UserDefaults.standard.set(5, forKey: "tipoCircular")
                UserDefaults.standard.set(1, forKey: "noLeido")
                self.actualizaLeidosCirculares(idCircular: Int(c.id),idUsuario: Int(self.idUsuario)!)
                performSegue(withIdentifier: "NotifSegue", sender: self)
                //self.tableViewCirculares.reloadData()
                //
                }
            }else{
        ...
        func borrarCirculares(){
            ...
            let q = "DELETE FROM appNotificacionCHMD"
                var statement:OpaquePointer?
                        ...
                        for index in 0...((datos).count) - 1
                        {
                            ...
                            var imagen:UIImage
                               ...
                               if(Int(favorito)==0 && Int(leido)==0){
                                    self.circulares.append(CircularTodas(id:Int(id)!,imagen: imagen,encabezado: "",nombre: titulo,fecha: fecha,estado: 0,contenido:"",adjunto:adj,fechaIcs: fechaIcs,horaInicialIcs: horaInicioIcs,horaFinalIcs: horaFinIcs, nivel:nv ?? "",noLeido:1,favorita: Int(favorito)!))
                               }
      ...
      @objc func leer(){
        ...
        func actualizaFavoritosCirculares(idCircular:Int,idUsuario:Int){
            ...
            if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                ...
                self.circulares.removeAll()
                let query = "UPDATE appCircularCHMD SET favorita=1 WHERE idCircular=? AND idUsuario=? AND tipo=2"
        ...
        func eliminaFavoritosCirculares(idCircular:Int,idUsuario:Int){
            ...
            if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                ...
                self.circulares.removeAll()
                let query = "UPDATE appCircularCHMD SET favorita=0 WHERE idCircular=? AND idUsuario=? AND tipo=2"
        ...
        func actualizaLeidosCirculares(idCircular:Int,idUsuario:Int){
            ...
            if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                ...
                self.circulares.removeAll()
                let query = "UPDATE appCircularCHMD SET leida=1 WHERE idCircular=? AND idUsuario=? AND tipo=2"
...
/*UIViewController,UITableViewDelegate, UITableViewDataSource,UISearchBarDelegate,UIGestureRecognizerDelegate,UITableViewDataSourcePrefetching {
    ...
    func leerNotificaciones(){
         ...
         */
           ...
           let consulta = "SELECT idCircular,nombre,textoCircular,leida,favorita,eliminada,created_at,fechaIcs,horaInicioIcs,horaFinIcs,nivel,adjunto  FROM appNotificacionCHMD WHERE leida=0"
           var queryStatement: OpaquePointer? = nil
        var imagen:UIImage
        imagen = UIImage.init(named: "appmenu05")!
        ...
        if sqlite3_prepare_v2(db, consulta, -1, &queryStatement, nil) == SQLITE_OK {
             ...
             while(sqlite3_step(queryStatement) == SQLITE_ROW) {
                ...
                if(eliminada==0 ){
                   self.circulares.append(CircularTodas(id:Int(id),imagen: imagen,encabezado: "",nombre: titulo,fecha: fechaCircular,estado: 0,contenido:cont.replacingOccurrences(of: "&#92", with: ""),adjunto:Int(adj),fechaIcs:fechaIcs,horaInicialIcs: hIniIcs,horaFinalIcs: hFinIcs, nivel:nivel,noLeido:1,favorita: Int(favorita)))
                }
      ...
      if ConexionRed.isConnectedToNetwork() == true {
        ...
        func tableView(tableView: UITableView, willDisplayCell cell: UITableViewCell, forRowAtIndexPath indexPath: NSIndexPath) {
            ...
            if indexPath.row == ultimo {
                ...
                let address=self.urlBase+self.metodoCirculares+"?usuario_id=\(self.idUsuario)"
               guard let _url = URL(string: address) else { return };
               self.getDataFromURLNotificaciones(url: _url)
                print("se pasó el último registro")
                }
        }
        ...
        func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
            ...
            if (indexPath.item >= 0 || indexPath.item < circulares.count) {
                ...
                print("notif. no leida \(c.noLeido)")
        ...
        func leerCirculares(){
             ...
             */
               ...
               let consulta = "SELECT idCircular,nombre,textoCircular,leida,favorita,eliminada,created_at,fechaIcs,horaInicioIcs,horaFinIcs,nivel,especiales  FROM appNotificacionCHMD"
               var queryStatement: OpaquePointer? = nil
            var imagen:UIImage
            imagen = UIImage.init(named: "appmenu05")!
        ...
        func borraCirculares(idCircular:Int,idUsuario:Int){
            ...
            if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                ...
                let query = "UPDATE appNotificacionCHMD SET eliminada=1,favorita=0 WHERE idCircular=? AND idUsuario=?"
        ...
        func eliminaFavoritosCirculares(idCircular:Int,idUsuario:Int){
            ...
            if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                ...
                self.circulares.removeAll()
                let query = "UPDATE appNotificacionCHMD SET favorita=0 WHERE idCircular=? AND idUsuario=?"
        ...
        func actualizaLeidosCirculares(idCircular:Int,idUsuario:Int){
            ...
            if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                ...
                self.circulares.removeAll()
                let query = "UPDATE appNotificacionCHMD SET leida=1 WHERE idCircular=? AND idUsuario=?"
        ...
        func actualizaNoLeidosCirculares(idCircular:Int,idUsuario:Int){
            ...
            if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                ...
                self.circulares.removeAll()
                let query = "UPDATE appNotificacionCHMD SET leida=0 WHERE idCircular=? AND idUsuario=?"
        ...
        func guardarCirculares(idCircular:Int,idUsuario:Int,nombre:String, textoCircular:String,no_leida:Int, leida:Int,favorita:Int,compartida:Int,eliminada:Int,fecha:String,fechaIcs:String,horaInicioIcs:String,horaFinIcs:String,nivel:String,adjunto:Int,especiales:String){
            ...
            if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                ...
                let query = "INSERT INTO appNotificacionCHMD(idCircular,idUsuario,nombre,textoCircular,no_leida,leida,favorita,eliminada,created_at,fechaIcs,horaInicioIcs,horaFinIcs,nivel,especiales) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
                if sqlite3_prepare(db,query,-1,&statement,nil) != SQLITE_OK {
                    print("Error")
                }
...
/*UIViewController,UITableViewDelegate, UITableViewDataSource,UISearchBarDelegate,UIGestureRecognizerDelegate,UITableViewDataSourcePrefetching {
    func tableView(_ tableView: UITableView, prefetchRowsAt indexPaths: [IndexPath]) {
       ...
       @IBOutlet weak var barBusqueda: UISearchBar!
        ...
        func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
            let eliminaAction = self.contextualDelAction(forRowAtIndexPath: indexPath)
            let swipeConfig = UISwipeActionsConfiguration(actions: [eliminaAction])
            return swipeConfig
        }
        ...
        //Leer las notificaciones cuando no haya internet
        func leerNotificaciones(){
            print("Leer desde la base de datos local")
            let fileUrl = try!
                       FileManager.default.url(for: .documentDirectory, in: .userDomainMask, appropriateFor: nil, create: false).appendingPathComponent("chmd_db1b.sqlite")
            ...
            let fileUrl = try!
                   ...
                   if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                       print("Error en la base de datos")
                            ...
                            var statement:OpaquePointer?
                    let query = "DELETE FROM appNotificacionCHMD";
                    if sqlite3_prepare(db,query,-1,&statement,nil) != SQLITE_OK {
                        print("Error")
                    }
                    if sqlite3_step(statement) == SQLITE_DONE {
        ...
        func actualizaFavoritosCirculares(idCircular:Int,idUsuario:Int){
            ...
            if(sqlite3_open(fileUrl.path, &db) != SQLITE_OK){
                ...
                self.circulares.removeAll()
                let query = "UPDATE appNotificacionCHMD SET favorita=1 WHERE idCircular=? AND idUsuario=?"
     ...
     func delCircular(direccion:String, usuario_id:String, circular_id:String){
        ...
        //Preguntar
        let dialogMessage = UIAlertController(title: "CHMD", message: "¿Deseas eliminar esta notificación?", preferredStyle: .alert)
